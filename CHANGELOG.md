...
- Asking for the compatibility renderer with `?splatBackend=spark` and then not getting it no longer ends in a blank viewport with nothing said. The app now warns that the renderer could not be loaded and offers the way out — remove the setting and let the app choose a renderer, or reload. Forcing WebGPU and failing already warned; forcing the compatibility renderer was silent.
